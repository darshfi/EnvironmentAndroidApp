...
fun SignupPage(modifier: Modifier = Modifier, navController: NavController, authViewModel: AuthViewModel) {
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Password),
            trailingIcon = {
                val icon = if (cpasswordVisible) Icons.Filled.Visibility else Icons.Filled.VisibilityOff
                IconButton(onClick = { cpasswordVisible = !cpasswordVisible }) {
                    Icon(imageVector = icon, contentDescription = if (cpasswordVisible) "Hide password" else "Show password")
                }
            },
            colors = OutlinedTextFieldDefaults.colors(
                focusedBorderColor = GreenGrey30,
        ...
        colors = ButtonDefaults.buttonColors(
            ...
            enabled = email.isNotEmpty() && password.isNotEmpty() && cpassword.isNotEmpty()
        ){
